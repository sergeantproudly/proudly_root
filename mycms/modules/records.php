...
define('DOCUMENT_TOPICS_ID',90);
define('DOCUMENT_POSTS_ID',91);
...
class records extends krn_abstract{
	...
	function Browse($document=array(),$params=array()){
				...
				switch($info['Type']){
					...
					default:
						if($document['LeadElement']==$name || (!$document['LeadElement'] && ($name=='Title' || $name=='Name'))){
							$tdItems[]='<a href="index.php?module=records&mode=view&document_id='.$document['Id'].'&record_id='.$rec[$document['IdElement']].'">'.htmlspecialchars($rec[$name],ENT_QUOTES).'</a>';
						}else{
		...
		if($params['parent_record_field'] && $params['parent_record_id']){
			$toolAdd=strtr(LoadTemplate('tool_add'),array(
				'<%ACTION%>'	=> 'recAdd(this,'.$document['Id'].', {parent_record_id: '.$params['parent_record_id'].','.($params['subst_record_id']?'subst_record_id: '.$params['subst_record_id'].',':'').' parent_record_field: \''.$params['parent_record_field'].'\'});'
			));
			$toolEdit=GetButton('Изменить',false,'recEdit(this,'.$document['Id'].', {parent_record_id: '.$params['parent_record_id'].','.($params['subst_record_id']?'subst_record_id: '.$params['subst_record_id'].',':'').' parent_record_field: \''.$params['parent_record_field'].'\'});');
		...
		foreach($fields as $name=>$info){
			...
			switch($info['Type']){
				...
				case 4:
					...
					$data='<a href="'.ROOT_DIR.$record[$name].'" target="_blank"><img src="'.ROOT_DIR.$record[$name].'" alt="" style="max-height: 300px; max-width: 300px;"/></a>';
				break;
				case 5:
	...
	function BrowseRecordChilds(){
		...
		while($rec=dbGetRecord($res)){
			...
			if($rec['Id']==DOCUMENT_TOPICS_ID || $rec['Id']==DOCUMENT_POSTS_ID)$params['subst_record_id']=$this->record['PhpbbId'];
